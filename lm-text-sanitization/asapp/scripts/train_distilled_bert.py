...
def distill_bert(
    ...
    configuration['hidden_size'] = new_embedding_size #La cantidad de tinybert es 128
    configuration['intermediate_size'] = int(new_embedding_size*(old_intermediate_size/old_embedding_size)) #La cantidad de tinybert es 128
...
model_name = "falbaneseASAPP/modelo_prueba_1"
repo_name = get_full_repo_name(model_name, token="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")
...
num_update_steps_per_epoch = train_size #len(train_dataloader)
...
for epoch in range(num_train_epochs):
    ...
    losses = losses[: test_size]#len(eval_dataset)]
    try:
